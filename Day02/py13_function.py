...
# 리턴값이 없는 함수
def plus(x, y) -> None: # 리턴값이 없으므로 None == void
    print(x + y)
...
'''
# 다중 파라미터
def add_many(*args): # C/C++ pointer처럼 보이지만 아님, 여러개의 변수를 받음
    result = 0
    for i in args:
        result += i
...
# print(add_many('Life', 'is', 'short'))

# 키워드 매개변수 = 결과가 딕셔너리 형태로 나옴
...
introduce_myself(man=False, name='애슐리', age=40) # 파라미터명 지정하면 순서 상관없음
...
def valtest2():
    global val #전역번수 val을 내가 함수내에서 잠시 가져다쓰겠다!
    val += 10
